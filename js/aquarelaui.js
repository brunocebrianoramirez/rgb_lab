/* ============================================================
   ...
   da filmadora: cada peça é uma função.
     ...
     os pincéis ........ quatro redondos (n.º 2, 6, 12 e 24), e o
                         TAMANHO livre por baixo deles — até bem grande
     + − (no vidro) .... o zoom da folha; com zoom, barras discretas
                         de rolagem (e Ctrl + roda sobre a folha)
...
(function (VE) {
  ...
  function tamanhoDaFolha() {
    var p = VE.project, cw = p ? p.canvas.w : 1920, ch = p ? p.canvas.h : 1080, lado = definicao(cw, ch);
      ...
      '<div class="aq-conjunto" id="aqConjunto">' +
        '<div class="aq-mesa" id="aqMesa">' +
          ...
          '<div class="aq-caixa" id="aqCaixa">' +
            ...
            '<div class="aq-tamanho" title="O tamanho do pincel, em pixels da folha ([ e ] também)"><label>TAMANHO</label>' +
              '<input type="range" id="aqTamanho" min="1" max="300" step="1" value="10"><input type="number" id="aqTamanhoN" min="1" max="300" step="1" value="10"></div>' +
          ...
          '<div class="aq-vidro" id="aqVidro">' +
            ...
            '<div class="aq-papel" id="aqPapel"><div class="aq-folha-caixa" id="aqFolhaCaixa"><canvas id="aqFolha"></canvas><canvas class="aq-folhear hidden" id="aqFolhear"></canvas></div>' +
              '<div class="aq-cursor" id="aqCursor"></div></div>' +
            '<div class="aq-zoom" id="aqZoom">' +
              '<button id="aqZoomMenos" title="Afastar (Ctrl + roda)">−</button>' +
              '<button id="aqZoomTxt" title="Voltar a caber inteira">100%</button>' +
              '<button id="aqZoomMais" title="Aproximar (Ctrl + roda)">+</button></div>' +
          ...
          '<div class="aq-coluna">' +
            '<div class="aq-luz" id="aqLuz" title="A mesa de luz: arraste. Quanto do vídeo aparece por baixo do papel; no fim do curso a luz vem por trás da pintura">' +
              '<i class="aq-knob"><b></b></i><small id="aqLuzTxt">LUZ 55%</small></div>' +
  ...
  var ZOOMS = [1, 1.5, 2, 3, 4, 6, 8];
  /* muda o zoom mantendo fixo o ponto da folha que está sob (cx, cy) —
     o centro da janela, ou o ponteiro                                 */
  function zoomPara(z, cx, cy) {
    var pap = el('aqPapel'), r = pap.getBoundingClientRect();
    z = Math.max(1, Math.min(8, z));
    if (cx == null) { cx = r.left + r.width / 2; cy = r.top + r.height / 2; }
    var antes = est.zoom || 1;
    var fx = (cx - r.left + pap.scrollLeft) / antes, fy = (cy - r.top + pap.scrollTop) / antes;
    est.zoom = z;
    ajustarVidro();
    pap.scrollLeft = fx * z - (cx - r.left); pap.scrollTop = fy * z - (cy - r.top);
    est.redesenha = true;
  }
  function zoomPasso(dir, cx, cy) {
    var z = est.zoom || 1, i = 0, melhor = 1e9;
    for (var k = 0; k < ZOOMS.length; k++) { var d = Math.abs(ZOOMS[k] - z); if (d < melhor) { melhor = d; i = k; } }
    i = Math.max(0, Math.min(ZOOMS.length - 1, i + dir));
    zoomPara(ZOOMS[i], cx, cy);
  }
  ...
  function raioMax() { return M ? Math.max(40, Math.round(M.w * 0.3)) : 300; }
  function pintarPinceis() {
    el('aqPinceis').innerHTML = PINCEIS.map(function (p, i) {
      return '<button class="aq-pincel' + (Math.abs(est.raio - p.raio) < 0.01 && est.ferr === 'pincel' ? ' on' : '') + '" data-pincel="' + i + '" style="--n:' + (i + 1) + '" title="Pincel redondo n.º ' + p.n + '"><i></i><small>' + p.n + '</small></button>';
    }).join('');
    var r = el('aqTamanho'), n = el('aqTamanhoN'), mx = raioMax();
    r.max = mx; n.max = mx; r.value = est.raio; n.value = Math.round(est.raio);
    r.style.setProperty('--fill', ((est.raio - 1) / (mx - 1) * 100).toFixed(1) + '%');
  ...
  function aplicarFerramenta() {
    if (!M) return;
    var r = est.raio, dil = DILUICAO[est.diluicao] || DILUICAO.media;
  ...
  function pintarLuz() {
    ...
    el('aqLuzTxt').textContent = k <= 0.7 ? 'LUZ ' + Math.round(k / 0.7 * 100) + '%' : 'RETRO ' + Math.round((k - 0.7) / 0.3 * 100) + '%';
    if (!M) return;
  ...
  function usar() {
    ...
    A.guardarAtual().then(function () {
      ...
      /* a tela da composição nasce da primeira fonte, e uma fonte pequena
         deixa a tela pequena (160×90 já aconteceu): a aquarela não pode
         ser esmagada nela. A folha tem a proporção da composição, então
         crescer a tela até a folha não muda o enquadramento de ninguém. */
      var pc = VE.project.canvas, telaAntes = pc.w + '×' + pc.h;
      if (!primeiro && (pc.w < s.w || pc.h < s.h)) {
        VE.setCanvas(Math.max(pc.w, s.w), Math.max(pc.h, s.h));
        toast('a tela da composição era ' + telaAntes + ' — foi para ' + VE.project.canvas.w + '×' + VE.project.canvas.h + ', a definição da folha, para a aquarela não perder qualidade');
      }
  ...
  function ligar() {
    ...
    function cursor(ev) {
      var r = folha.getBoundingClientRect(), esc = r.width / M.w, pap = el('aqPapel'), pr = pap.getBoundingClientRect();
      cur.style.left = (ev.clientX - pr.left + pap.scrollLeft) + 'px'; cur.style.top = (ev.clientY - pr.top + pap.scrollTop) + 'px';
    ...
    el('aqPinceis').addEventListener('click', function (ev) {
      ...
      est.pincel = +b.dataset.pincel; est.raio = PINCEIS[est.pincel].raio;
      if (est.ferr !== 'seco') est.ferr = 'pincel';
      aplicarFerramenta(); pintarPinceis();
    });
    /* o tamanho livre: a linha é o campo */
    function tamanho(v) { est.raio = Math.max(1, Math.min(raioMax(), v)); aplicarFerramenta(); pintarPinceis(); }
    el('aqTamanho').addEventListener('input', function () { tamanho(parseFloat(this.value)); });
    el('aqTamanhoN').addEventListener('change', function () { var v = parseFloat(this.value); if (isFinite(v)) tamanho(v); });
    /* o zoom */
    el('aqZoomMais').addEventListener('click', function () { zoomPasso(1); });
    el('aqZoomMenos').addEventListener('click', function () { zoomPasso(-1); });
    el('aqZoomTxt').addEventListener('click', function () { zoomPara(1); });
    el('aqPapel').addEventListener('wheel', function (ev) {
      if (!(ev.ctrlKey || ev.metaKey)) return;                /* sem Ctrl a roda rola a folha */
      ev.preventDefault();
      zoomPasso(ev.deltaY < 0 ? 1 : -1, ev.clientX, ev.clientY);
    }, { passive: false });
    ...
    var luz = el('aqLuz');
    /* arrastar para a DIREITA ou para CIMA acende; para a esquerda ou para
       baixo apaga — os dois eixos, porque quem gira um botão puxa para o
       lado que lhe parecer natural                                      */
    luz.addEventListener('pointerdown', function (ev) {
      var x0 = ev.clientX, y0 = ev.clientY, k0 = est.luzKnob;
      function mv(e) { est.luzKnob = Math.max(0, Math.min(1, k0 + ((e.clientX - x0) + (y0 - e.clientY)) / 180)); pintarLuz(); }
