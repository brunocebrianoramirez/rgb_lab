...
(function (VE) {
  ...
  var FS_VEL = [
    ...
    /* contorno: a face com uma célula seca não deixa passar água — e a
       beira da folha é parede (a última coluna mandava água e pigmento
       para fora da folha: −1,3% do pigmento em 200 passos, medido)     */
    '  if(R.a < 0.5 || ij.x == uTam.x - 1) u = 0.0;',
    '  if(U.a < 0.5 || ij.y == uTam.y - 1) v = 0.0;',
  ...
  var FS_PIG = [
    ...
    /* a face de fora da folha é parede: a primeira coluna não recebe de si mesma */
    '  float uL = (ij.x == 0) ? 0.0 : texelFetch(uAgua, qL, 0).r, vD = (ij.y == 0) ? 0.0 : texelFetch(uAgua, qD, 0).g;',
  ...
  var FS_CAP = [
    ...
    'uniform float uAlfa, uEps, uSigma, uEvap, uEta, uSecaP, uSalBebe, uSangra, uTensao;',
    ...
    '  ivec2 qL = CL(ij + ivec2(-1,0)), qR = CL(ij + ivec2(1,0)), qD = CL(ij + ivec2(0,-1)), qU = CL(ij + ivec2(0,1));',
    '  float wL = texelFetch(uSat, qL, 0).a, wR = texelFetch(uSat, qR, 0).a, wD = texelFetch(uSat, qD, 0).a, wU = texelFetch(uSat, qU, 0).a;',
    '  bool ML = texelFetch(uAgua, qL, 0).a > 0.5, MR = texelFetch(uAgua, qR, 0).a > 0.5, MD = texelFetch(uAgua, qD, 0).a > 0.5, MU = texelFetch(uAgua, qU, 0).a > 0.5;',
    /* a TENSÃO SUPERFICIAL do filme: uma troca de água entre vizinhas
       molhadas, do MESMO campo (conservativa, simétrica, e a célula nunca
       dá mais de 4×uTensao do que tem), que achata o que é do tamanho de
       uma célula. É o modo que explodia: a onda de gravidade da malha só é
       estável até uma altura de água (~0,79 sem isto — medido: a 1,08 a
       água virava um tabuleiro de xadrez em 25 passos, as "bolinhas");
       com a tensão o limiar sobe, e a folha só segura uWMax           */
    '  if(M > 0.5){',
    '    float lap = 0.0;',
    '    if(ML && ij.x > 0) lap += wL - w;',
    '    if(MR && ij.x < uTam.x - 1) lap += wR - w;',
    '    if(MD && ij.y > 0) lap += wD - w;',
    '    if(MU && ij.y < uTam.y - 1) lap += wU - w;',
    '    w = max(w + uTensao*lap, 0.0);',
    '  }',
    ...
    '  float wv = max(max(wL, wR), max(wD, wU));',
    '  if(M > 0.5){',
  ...
  var FS_TOOL = [
    ...
    /* a folha só segura tanta água (uWMax): o filme enche até o teto e
       para — cada passada por cima punha mais um morro de água, sem fim, e
  ...
  var GL_KM = [
    ...
    'uniform int uAtivo[8];',      /* 1 = este godê já pintou nesta folha; os outros o laço pula */
    ...
    /* o desenho e a saída amostram pelo uv, com filtro bilinear, em qualquer
       resolução: é o que deixa a folha ser DESENHADA maior que a simulação
       sem degraus. As espessuras levam quatro toques em quincôncio de meio
       texel (uSuave) — uma suavização de um texel, só no que se vê.      */
    /* a espessura chega já SOMADA (depositado + em suspensão) em duas
       texturas de 16 bits, feitas pela pré-passada (FS_PRE) na resolução da
       simulação: desenhar a 2× com as de 32 bits custava 35 ms só de
       largura de banda (16 leituras de 16 bytes por pixel) — medido    */
    'uniform float uSuave;',
    'vec4 espessura(sampler2D x, vec2 uv){',
    '  if(uSuave < 0.01) return texture(x, uv);',
    '  vec2 t = uSuave*0.5/vec2(uTam);',
    '  vec2 o1 = vec2(t.x, t.y), o2 = vec2(-t.x, t.y);',
    '  return (texture(x, uv + o1) + texture(x, uv - o1) + texture(x, uv + o2) + texture(x, uv - o2))*0.25;',
    '}',
    'vec3 papelBase(vec2 uv, sampler2D pap){',
    '  vec2 t = 1.0/vec2(uTam);',
    '  vec4 pp = texture(pap, uv);',
    '  float hR = texture(pap, uv + vec2(t.x, 0.0)).r, hU = texture(pap, uv + vec2(0.0, t.y)).r;',
    '  float sh = 1.0 + 0.12*(pp.r - hR) + 0.08*(pp.r - hU) + 0.02*(pp.b - 0.5);',
  ...
  var FS_VER = [
    ...
    'void main(){',
    '  vec2 uv = vUv;',
    '  vec4 x0 = espessura(uX0, uv);',
    '  vec4 x1 = espessura(uX1, uv);',
    '  vec3 base = papelBase(uv, uPapel);',
    '  vec3 fundo = base;',
    ...
    '  float molh = texture(uMolh, uv).r*uMostraAgua;',
    '  R *= (1.0 - 0.09*molh);',
  ...
  /* 8b. a pré-passada do desenho: soma depositado + suspenso por godê e o
     molhado à vista, na resolução da simulação, em 16 bits — é o que o
     desenho e a saída amostram, em qualquer escala                        */
  var FS_PRE = [
    PRE,
    'uniform sampler2D uDep0, uDep1, uPig0, uPig1, uAgua, uSat;',
    'layout(location=0) out vec4 oX0;',
    'layout(location=1) out vec4 oX1;',
    'layout(location=2) out vec4 oMolh;',
    'void main(){',
    '  ivec2 ij = IJ();',
    '  oX0 = texelFetch(uDep0, ij, 0) + texelFetch(uPig0, ij, 0);',
    '  oX1 = texelFetch(uDep1, ij, 0) + texelFetch(uPig1, ij, 0);',
    '  vec4 a = texelFetch(uAgua, ij, 0); float w = texelFetch(uSat, ij, 0).a;',
    '  oMolh = vec4(a.a*clamp(w*3.0, 0.0, 1.0), w, 0.0, 1.0);',
    '}'
  ].join('\n');
  ...
  var FS_EXP = [
    PRE,
    'uniform sampler2D uX0, uX1, uPapel;',
    'uniform int uModo;',     /* 0 = sobre o vídeo (C_w + alfa) · 1 = no papel (opaco) */
    GL_KM,
    'out vec4 o;',
    'void main(){',
    '  vec2 uv = vec2(vUv.x, 1.0 - vUv.y);',              /* PNG: a linha 0 é o topo */
    '  vec4 x0 = espessura(uX0, uv);',
    '  vec4 x1 = espessura(uX1, uv);',
    '  vec3 Tt;',
    '  if(uModo == 0){',
    ...
    '    o = vec4(aSrgb(Cw), cob);',
    '  } else {',
    '    vec3 R = pilha(x0, x1, papelBase(uv, uPapel), Tt);',
    '    o = vec4(aSrgb(R), 1.0);',
    '  }',
    '}'
  ].join('\n');
  ...
  function Motor(canvas, w, h, opts) {
    ...
    /* o desenho e a saída amostram as texturas de estado com filtro bilinear
       (a folha desenhada maior que a simulação, sem degraus); as passadas
       da simulação usam texelFetch e não sentem o filtro                */
    ...
    this.ferr = 'pincel';
    this.slot = 0;
    /* quais godês já pintaram nesta folha: o desenho e a saída só compõem
       esses (medido: o custo da pilha é por godê, 8 custava 35 ms a 2×) */
    this.ativos = [0, 0, 0, 0, 0, 0, 0, 0];
    ...
    this.par = {
      ...
      /* a água que a folha segura (o pincel enche o filme até aqui) e a
         tensão superficial do filme — os dois juntos mantêm a água abaixo
         do limiar em que a malha explode (14/09, o xadrez do Bruno)   */
      wMax: 0.8, tensao: 0.10
    };
  ...
  Motor.prototype.mkTex = function (fmt) {
    ...
    var flt = (fmt === gl.RGBA32F && !this.linear) ? gl.NEAREST : gl.LINEAR;
    if (fmt === gl.RGBA16F) { gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR); gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR); gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE); gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE); gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA16F, this.w, this.h, 0, gl.RGBA, gl.HALF_FLOAT, null); return t; }
  ...
  Motor.prototype.passar = function (pr, entradas, alvos, arma, tam) {
    var gl = this.gl;
    gl.useProgram(pr.p);
    if (alvos) {
    ...
    if (tam) gl.viewport(0, 0, tam.w, tam.h);
    else if (alvos) gl.viewport(0, 0, this.w, this.h);
  ...
  Motor.prototype.preDesenho = function () {
    var T = this.tex;
    this.passar(this.prog('pre', FS_PRE), { uDep0: T.dep0[0], uDep1: T.dep1[0], uPig0: T.pig0[0], uPig1: T.pig1[0], uAgua: T.agua[0], uSat: T.sat[0] }, [T.x0, T.x1, T.molh]);
  };
  ...
  Motor.prototype.armarKM = function (u, gl) {
    ...
    if (u.uAtivo) gl.uniform1iv(u.uAtivo, this.ativos);
  ...
  Motor.prototype.passo = function () {
    ...
    this.passar(this.prog('cap', FS_CAP), { uSat: T.sat[0], uAgua: T.agua[0], uPapel: T.papel, uMasc: T.masc[0] }, [T.sat[1], T.agua[1]], function (u) {
      ...
      gl.uniform1f(u.uSangra, P.floradas ? P.sangra : 9.0); gl.uniform1f(u.uTensao, P.tensao);
    });
  ...
  Motor.prototype.instantaneo = function () {
    ...
    n.molhada = this.molhada; n.ultimoToque = this.ultimoToque; n.ativos = this.ativos.slice();
    return n;
  };
  Motor.prototype.repor = function (n) {
    ...
    if (n.ativos) this.ativos = n.ativos.slice();
  };
  ...
  /* ------------------------------------------------------- desenhar
     A folha pode ser desenhada MAIOR que a simulação (até 2×): o canvas
     cresce, o desenho amostra por uv com filtro bilinear e a suavização —
     é o que tira os degraus quando a mesa está com zoom.               */
  Motor.prototype.escalaTela = function (esc) {
    esc = Math.max(1, Math.min(2, esc || 1));
    var W = Math.round(this.w * esc), H = Math.round(this.h * esc);
  ...
  /* --------------------------------------------- a saída do quadro */
  Motor.prototype.exportar = function (modo, W, H) {
    var self = this, gl = this.gl, T = this.tex;
    W = Math.max(8, Math.min(Math.round(this.w * 2), Math.round(W || this.w)));
    H = Math.max(8, Math.min(Math.round(this.h * 2), Math.round(H || this.h)));
    var alvo = T.u8[0], tam = null;
    if (W !== this.w || H !== this.h) {
      /* um alvo do tamanho pedido, guardado enquanto o tamanho não muda */
      if (!this.texExp || this.texExpW !== W || this.texExpH !== H) {
        if (this.texExp) gl.deleteTexture(this.texExp);
        this.texExp = gl.createTexture(); gl.bindTexture(gl.TEXTURE_2D, this.texExp);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR); gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE); gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA8, W, H, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
        this.texExpW = W; this.texExpH = H;
      }
      alvo = this.texExp; tam = { w: W, h: H };
    }
    this.preDesenho();
    this.passar(this.prog('exp', FS_EXP), { uX0: T.x0, uX1: T.x1, uPapel: T.papel8 }, [alvo], function (u) {
      self.armarKM(u, gl); gl.uniform1i(u.uModo, modo === 'papel' ? 1 : 0);
      gl.uniform1f(u.uSuave, self.suave);
    }, tam);
    var out = new Uint8Array(W * H * 4);
    gl.readPixels(0, 0, W, H, gl.RGBA, gl.UNSIGNED_BYTE, out);
    out.w = W; out.h = H;
  ...
  Motor.prototype.desempacotar = function (bytes) {
    ...
    this.limpar();
    /* quais godês têm pigmento: um canal por godê, nos dois RGBA8 */
    for (var k = 0; k < 8; k++) {
      var ini = (k < 4 ? 0 : n) + (k & 3), fim = (k < 4 ? n : 2 * n), achou = 0;
      for (var i = ini; i < fim; i += 4) { if (bytes[i]) { achou = 1; break; } }
      this.ativos[k] = achou;
    }
  ...
  /* `saida` é o tamanho do quadro que vai para o laboratório: o da
     composição (até 2× a simulação), para não haver reescala depois     */
  A.filme = { quadros: [], atual: 0, fps: 12, passo: 2, inicio: 0, w: 0, h: 0, modo: 'video', saida: null };
  ...
  A.guardarAtual = function () {
    ...
    var sw = (F.saida && F.saida.w) || m.w, sh = (F.saida && F.saida.h) || m.h;
    var estado = m.empacotar(), rgba = m.exportar(F.modo, sw, sh);
    q.w = rgba.w; q.h = rgba.h; q.sujo = false;
    return Promise.all([comprimir(estado, 'deflate'), A.png(rgba.w, rgba.h, rgba)]).then(function (r) {
  ...
  A.novoFilme = function (w, h, saida) {
    var F = A.filme;
    F.quadros.forEach(function (q) { if (q.bitmap) { try { q.bitmap.close(); } catch (e) { } } });
    F.quadros = [{}]; F.atual = 0; F.w = w; F.h = h; F.saida = saida || null;
  };
  ...
  A.empacotarRolo = function (quadros, fps, w, h, paleta) {
    var cab = JSON.stringify({
      ...
      paleta: paleta || null,
      /* os PNGs têm w×h (a saída); os estados têm o tamanho da simulação */
      sim: A.motor ? [A.motor.w, A.motor.h] : [w, h],
